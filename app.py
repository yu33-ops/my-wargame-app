...
with col_right:
    ...
    if run_button:
        ...
        ai_prompt = f"你是一位军事题材的小说家和战演场景主笔。请根据以下真实的推演参数，充分展开你的想象力，写一段 300 到 400 字、极具硝烟感、画面极其震撼 and 详实的现代战争想定场景描述。【交战背景】：红方派遣【{red_summary}】在【{keywords}】的恶劣环境中发起强攻；蓝方迅速出动【{blue_summary}】进行针对性迎击。【真实推演判定】：在这场冲突中，最终的结果判定为【{winner}】。【写作红线要求】：1. 绝对不要包含任何 JSON、数字统计或错误代码，纯写具有强烈临场感的交战文学描述。2. 必须重点突出【{keywords}】里的天气环境（如夜间、暴雨等）对双方士兵视线和先进雷达兵器的严重干扰干扰。3. 直接输出描述文本，不要写任何类似于“好的，这是为您生成的场景”等客套废话。"
        ...
        with st.spinner("🚀 AI大模型正在渲染多维高逼真战场想定剧本..."):
            ...
            # ==================== 📊 🔥 核心调整：左右两侧空出“隔离位”，强行让柱子瘦身 ====================
            st.subheader("📈 3. 红蓝双边经济损耗直观对比 (单位: 万元)")
            
            # 💡 黄金排版算法：我们在数据表的最前和最后，加入两个完全没有高度（0）的虚拟隐形列
            chart_dataframe = pd.DataFrame(
                {
                    "经济损失总额(万元)": [0, red_total_cost, blue_total_cost, 0]
                },
                index=["", "🔴 红方阵营", "🔵 蓝方阵营", " "]  # 用前后的空格占位，挤压核心柱体的横向空间
            )
            
            # 渲染图表：由于两边被空白隐形列强行挤压，红蓝两根柱子会瞬间变成非常精神、纤细、高级的高清立柱！
