    ...
    try:
        ...
        return {
            ...
            "蓝方阵营": {
                ...
                "PATRIOT": {"名称": "爱国者-3防空导弹", "类型": "防空反导", "单价_万元": 30000, "铁定权重": 110},
...
with col_right:
    ...
    if run_button:
        ...
        ai_prompt = f"你是一位军事题材的小说家 and 战演场景主笔。请根据以下真实的推演参数，充分展开你的想象力，写一段 300 到 400 字、极具硝烟感、画面极其震撼和详实的现代战争想定场景描述。【交战背景】：红方派遣【{red_summary}】在【{keywords}】的恶劣环境中发起强攻；蓝方迅速出动【{blue_summary}】进行针对性迎击。【真实推演判定】：在这场冲突中，最终的结果判定为【{winner}】。【写作红线要求】：1. 绝对不要包含任何 JSON、数字统计或错误代码，纯写具有强烈临场感的交战文学描述。2. 必须重点突出【{keywords}】里的天气环境（如夜间、暴雨等）对双方士兵视线和先进雷达兵器的严重干扰干扰。3. 直接输出描述文本，不要写任何类似于“好的，这是为您生成的场景”等客套废话。"
        ...
        with st.spinner("🚀 AI大模型正在渲染多维高逼真战场想定剧本..."):
            ...
            # ==================== 📊 🔥 终极必杀：通过序号矩阵算法，强制逼迫柱体靠左 ====================
            st.subheader("📈 3. 红蓝双边经济损耗直观对比 (单位: 万元)")
            
            # 💡 破解字典序重排的绝招：采用 01、02、03 前缀。
            # 01 和 02 权重最高，会被强制锁死在 X轴 最左边！03、04 等隐形占位列自动退居右侧，实现完美瘦身靠左！
            chart_dataframe = pd.DataFrame(
                {
                    "经济损失总额(万元)": [red_total_cost, blue_total_cost, 0, 0, 0]
                },
                index=["01.🔴 红方阵营", "02.🔵 蓝方阵营", "03. ", "04.  ", "05.   "]  
            )
